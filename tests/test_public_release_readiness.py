...
def _write_public_skeleton(root):
    ...
    _write(os.path.join(root, "docs", "RESULTS.md"), "# Results\n")
    _write(
        os.path.join(root, "release", "public_release_manifest.json"),
        json.dumps({"manifest_version": "unit", "artifacts": []}) + "\n",
    )
...
class PublicReleaseReadinessTests(unittest.TestCase):
    ...
    def test_internal_paths_and_key_incident_notes_are_blockers(self):
        with tempfile.TemporaryDirectory() as d:
            _write_public_skeleton(d)
            local_path = "/" + "Users" + "/example-user/demo"
            login_host = "cluster-" + "login1"
            ...
            self.assertIn("hpc_login_host", kinds)
    ...
    def test_synthetic_test_placeholders_do_not_warn(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            _write(
                ...
                "\n".join([
                    "cmd = 'ssh <hpc-login-host> cd /home/fs01/<user>/bio_sfm_smoke'",
                    "remote_spec = '<hpc-login-host>:/remote/root'",
                    "private_fixture = '<hpc-login-host>:/home/fs01/<user>/project'",
                    "",
                ]),
            )
    ...
    def test_real_hpc_context_in_tests_stays_warning(self):
        with tempfile.TemporaryDirectory() as d:
            _write_public_skeleton(d)
            real_hpc_path = "/" + "home" + "/" + "cluster" + "/" + "example-user" + "/project"
            _write(
                os.path.join(d, "tests", "test_fixture.py"),
                f"remote_root = '{real_hpc_path}'\n",
            )
            ...
            self.assertEqual(warnings[0]["kind"], "hpc_home_path")

    def test_tracked_only_include_results_scans_git_surface_not_scratch(self):
        with tempfile.TemporaryDirectory() as d:
            _write_public_skeleton(d)
            real_hpc_path = "/" + "home" + "/" + "cluster" + "/" + "example-user" + "/project"
            _write(os.path.join(d, "results", "tracked.json"), f"remote = '{real_hpc_path}'\n")
            ...
            self.assertFalse(rep["audit_ok"])
            kinds = {finding["kind"] for finding in rep["findings"]}
            self.assertIn("hpc_home_path", kinds)
