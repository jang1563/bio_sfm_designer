...
- external HPC submission allowed: false.

The release records the locked design in
`configs/m6d_w2c_one_shot_protocol.json`. Passing the design calculations only
shows that the sample plan is internally coherent and prospectively powered;
it does not authorize compute or support a W2c claim.
...
The public release stops before target-MSA execution and contains no W2c model
records. Any later compute belongs to a separately reviewed execution workflow.
