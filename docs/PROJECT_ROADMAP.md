...
6. Treat the current W2 panels as completed negative evidence, not missing runs. The latest durable
   ...
   emits the approval runbook, sync-back script, and panel-completion script. The next W2 decision is
   whether to explicitly approve the corrected 7-target v11 ProteinMPNN/Boltz panel; no v11 panel records
   exist yet.
