...
6. Treat the current W2 panels as completed negative evidence, not missing runs. The latest durable
   ...
   `python -m bio_sfm_designer.experiments.m6d_w2_panel_guarded_preflight --run-local-dry-run`. The next
   W2 decision is whether to explicitly approve the corrected 7-target v11 ProteinMPNN/Boltz panel; no
   v11 panel records exist yet.
