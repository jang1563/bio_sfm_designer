...
class PrecomputedScreen:
    """Local consume-side of the external HPC DeBERTa screen (docs/HPC.md).
    ...
    have been screened on external HPC, so a missing verdict is a pipeline gap, NOT a clearance. The
    ...
    def screen_target(self, spec: ObjectiveSpec) -> ScreenVerdict:
        ...
        if not allowed:
            return ScreenVerdict(False, decision, "builtin_policy", reason)
        v = self._verdicts.get("objective")  # optional: objective screened on external HPC under id "objective"
        if v and v.get("flag"):
