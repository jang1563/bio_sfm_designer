"""Validate or execute one authorized W6-v3.2 live shadow panel."""
...
from .w6_v32_transport_panel import load_and_validate_panel


SCOPE_SCHEMA = "w6_v32_live_scope_v1"
CAPTURE_SCHEMA = "w6_v32_live_capture_v1"
RECEIPT_SCHEMA = "w6_v32_live_receipt_v1"
_SCOPE_FIELDS = {
    "schema_version",
    "scope_id",
    "frozen_at",
...
def load_and_validate_scope(
    ...
    *,
    ...
    """Validate the frozen W6-v3.2 scope without constructing a provider."""
    ...
    if expected_scope_sha256 is not None:
        ...
        if observed_scope_sha256 != expected:
            raise ValueError("approved W6-v3.2 scope SHA-256 mismatch")
    scope = _load_json(scope_path)
    if not isinstance(scope, dict) or set(scope) != _SCOPE_FIELDS:
        raise ValueError("W6-v3.2 scope fields do not match the frozen schema")
    ...
    if scope["provider"] != "anthropic" or scope["model"] != "claude-opus-4-8":
        raise ValueError("W6-v3.2 must preserve the provider and model")
    if scope["mode"] != "shadow":
        raise ValueError("W6-v3.2 mode must be shadow")
    if scope["max_output_tokens_per_call"] != 512:
        raise ValueError("W6-v3.2 output cap must be exactly 512")
    if scope["sdk_retries_per_call"] != 0:
        raise ValueError("W6-v3.2 SDK retries must be zero")
    ...
    if any(scope[key] is not True for key in _REQUIRED_TRUE):
        raise ValueError("W6-v3.2 scope is missing a required true flag")
    if any(scope[key] is not False for key in _REQUIRED_FALSE):
        raise ValueError("W6-v3.2 scope is missing a required false flag")
    if scope["failure_telemetry_schema"] != TELEMETRY_SCHEMA:
        raise ValueError("W6-v3.2 failure telemetry schema mismatch")
    if scope["behavioral_change"] != "none":
        raise ValueError("W6-v3.2 cannot change provider behavior")
    if (
        scope["instrumentation_change"]
    ...
    if _sha256_file(panel_path) != scope["panel_sha256"]:
        raise ValueError("W6-v3.2 scope panel SHA-256 mismatch")
    if _sha256_file(request_path) != scope["request_sha256"]:
        raise ValueError("W6-v3.2 scope request SHA-256 mismatch")
    panel, _, _ = load_and_validate_panel(panel_path, repo_root=repo_root)
    if panel["panel_id"] != scope["panel_id"]:
        raise ValueError("W6-v3.2 scope panel id mismatch")
...
def capture_live_panel(
    *,
    ...
    """Capture one approved W6-v3.2 panel with safe failure telemetry."""
    ...
    if not scope["live_execution_authorized"]:
        raise PermissionError("W6-v3.2 live execution is not authorized")
    worktree_porcelain = _git_porcelain(repo_root)
    if worktree_porcelain:
        raise RuntimeError("W6-v3.2 live execution requires a clean worktree")
    ...
    metadata_complete = sum(row["transport_metadata_complete"] for row in capture_rows)
    telemetry_complete = sum(
        isinstance(row["failure_telemetry"], dict)
        and validate_failure_telemetry(row["failure_telemetry"]) is not None
        for row in capture_rows
    )
    output_limit_stops = sum(row["output_limit_stop"] for row in capture_rows)
    failure_reasons = Counter(
        row["failure_telemetry"]["safe_reason_code"]
        for row in capture_rows
    ...
    if (
        failed == 0
        and metadata_complete == len(capture_rows)
        and telemetry_complete == len(capture_rows)
    ):
        responses = [
            {
    ...
    if failed:
        status = "w6_v32_live_capture_incomplete_no_retry"
    elif metadata_complete != len(capture_rows):
        status = "w6_v32_live_capture_incomplete_transport_metadata"
    elif telemetry_complete != len(capture_rows):
        status = "w6_v32_live_capture_incomplete_failure_telemetry"
    else:
        status = "w6_v32_live_capture_complete_pending_review"
    input_counts = [
        row["input_tokens"]
        for row in capture_rows
    ...
    receipt = {
        ...
        "failure_telemetry_complete_calls": telemetry_complete,
        ...
        "instrumentation_change": scope["instrumentation_change"],
        "exception_messages_stored": False,
        "tracebacks_stored": False,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="validate or capture the W6-v3.2 live shadow panel"
    )
    ...
    validate_parser.add_argument(
        "--scope",
        type=_path,
        default=Path("configs/w6_v32_live_scope.json"),
    )
    ...
    if args.command == "validate":
        ...
        print(
            f"status=w6_v32_scope_valid scope_id={scope['scope_id']} "
    ...
    print(
        ...
        f"telemetry_complete={receipt['failure_telemetry_complete_calls']} "
