"""Apply hash-bound offline reviews to prospective W6-v3 live responses."""
...
ANNOTATION_SCHEMA = "w6_v3_prospective_review_annotations_v1"
RECEIPT_SCHEMA = "w6_v3_prospective_review_receipt_v1"
_ANNOTATION_FIELDS = {
    "schema_version",
    "source_panel_sha256",
...
def apply_review_annotations(
    *,
    ...
    """Apply complete reviews after exact source and scope validation."""
    ...
    if not isinstance(annotations, dict) or set(annotations) != _ANNOTATION_FIELDS:
        raise ValueError("annotation fields do not match the frozen schema")
    ...
    for key in ("identity", "type", "reviewed_at"):
        if not isinstance(reviewer[key], str) or not reviewer[key].strip():
            raise ValueError(f"reviewer.{key} must be a non-empty string")
    ...
    receipt = {
        ...
        "status": "prospective_offline_independent_review_complete",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="apply hash-bound W6-v3 prospective rubric annotations"
    )
