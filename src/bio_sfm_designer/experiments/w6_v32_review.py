"""Apply hash-bound offline reviews to complete W6-v3.2 live responses."""
...
from .w6_v32_transport_panel import (
    load_and_validate_panel,
    score_response_records,
)


ANNOTATION_SCHEMA = "w6_v32_review_annotations_v1"
RECEIPT_SCHEMA = "w6_v32_review_receipt_v1"
_ANNOTATION_FIELDS = {
    "schema_version",
    "source_panel_sha256",
...
def apply_review_annotations(
    *,
    ...
    """Apply complete reviews after exact W6-v3.2 source validation."""
    ...
    if not isinstance(annotations, dict) or set(annotations) != _ANNOTATION_FIELDS:
        raise ValueError("annotation fields do not match the W6-v3.2 schema")
    ...
    for row in annotation_rows:
        ...
        if (
            review["scope_tag"]
            not in cases_by_id[case_id]["expected"]["allowed_scope_tags"]
        ):
    ...
    receipt = {
        ...
        "status": "w6_v32_offline_independent_review_complete",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="apply hash-bound W6-v3.2 rubric annotations"
    )
    parser.add_argument("--repo-root", type=_path, default=Path("."))
    parser.add_argument(
        "--panel",
        type=_path,
        default=Path("configs/w6_v32_transport_panel.json"),
    )
    parser.add_argument(
        "--requests",
        type=_path,
        default=Path("results/w6_v32_transport_requests.jsonl"),
    )
