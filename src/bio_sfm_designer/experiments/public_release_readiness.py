...
HPC_HOME_PATH_PATTERN = re.compile(
    r"/home/[A-Za-z0-9._-]+/(?!<user>(?:/|\b))[A-Za-z0-9._-]+"
)
HPC_SCRATCH_PATH_PATTERN = re.compile(
    r"/scratch/(?!<user>(?:/|\b)|USER(?:/|\b))[A-Za-z0-9._-]+"
)
HPC_LOGIN_HOST_PATTERN = re.compile(
    r"(?<![A-Za-z0-9.-])(?!hpc-login-host\b)"
    r"[a-z0-9][a-z0-9.-]*-login[0-9]*\b",
    re.IGNORECASE,
)

INTERNAL_PATTERNS: Sequence[Tuple[str, Pattern[str]]] = [
    ("local_absolute_path", LOCAL_USER_PATH_PATTERN),
    ("hpc_home_path", HPC_HOME_PATH_PATTERN),
    ("hpc_scratch_path", HPC_SCRATCH_PATH_PATTERN),
    ("hpc_login_host", HPC_LOGIN_HOST_PATTERN),
    ("private_github_context", re.compile(r"\b" + re.escape(PRIVATE_GITHUB_PHRASE) + r"\b", re.IGNORECASE)),
]

INTERNAL_REPLACEMENTS = {
    "local_absolute_path": "/Users/<local-user>",
    "hpc_home_path": "/home/<site>/<user>",
    "hpc_scratch_path": "/scratch/<user>",
    "hpc_login_host": "<hpc-login-host>",
    "private_github_context": "<private-repository-context>",
}
...
def _redact_line(line: str) -> str:
    out = line.strip()
    for kind, pattern in INTERNAL_PATTERNS:
        out = pattern.sub(INTERNAL_REPLACEMENTS[kind], out)
...
def _claim_corpus(root: str) -> str:
    chunks = []
    for rel in ("README.md", "docs/RESULTS.md", "docs/PROJECT_ROADMAP.md"):
