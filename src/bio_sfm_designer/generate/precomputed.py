"""M3 generative front-end (consume-side): serve candidate designs produced by an external HPC
generate job (RFdiffusion / ProteinMPNN / ESM).

Same HPC pattern as predict/structure.py: heavy generation runs on external HPC (GPU) and
...
class PrecomputedGenerator:
    """Serve precomputed candidate designs. Single-pass over a fixed external HPC-produced set:
